/**
 * The Request Preview panel (Story 2.5, AC4/AC12).
 *
 ...
 * out). The Employee picks a Leave Type and a range, clicks Preview, and sees what the request would
 * cost before committing to it. Story 2.6 extends this `features/leave/` dir into full submission.
 *
 * --- The one rule this screen must never break (AD-2 / AC12) ---
 *
 * The client computes NOTHING. `leave_days`, `available_before` and `available_after` are rendered
 * AS-IS (the server already derived `available_after`); the excluded dates, their reasons and each
...
import { useLeaveTypes, usePreviewLeaveRequest } from '../../api'
...
export function RequestPreviewPanel() {
  ...
  function updateField(field: keyof typeof form, value: string) {
    ...
    // Editing any input invalidates the last preview: clear it so the shown cost never describes
    // inputs the user has since changed (code review 2026-07-13). The whole point of this screen
    // is to show the cost OF THE CURRENT REQUEST; a stale figure defeats it.
